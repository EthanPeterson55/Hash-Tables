...
void set_capacity(struct dynarray* da, int new_capacity){
...
void set_data(struct dynarray* da, void** new_data){
  da->data = new_data;
  return;
}
 ...
 */
void dynarray_insert(struct dynarray* da, void* val, int idx) {
  assert(da);
  ...
  if(da->data[idx] == NULL){
    ...
    struct list* bucket = da->data[idx];
    list_insert(bucket, val);
  }
}
 ...
 *     of `idx` must be between 0 (inclusive) and n (exclusive), where n is the
 *     number of elements stored in the arljlhhray.
