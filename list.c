...
** Post-Conditions: head node of linklist returned.
*********************************************************************/
...
struct node* get_head(struct list* list){
 ...
 /*********************************************************************
** Function: get_val()
** Description: returns value of a specific node.
** Parameters: struct node* node
** Pre-Conditions: hash table created.
** Post-Conditions: value of node returned or null
*********************************************************************/
 ...
 */
void list_remove(struct list* list, void* val, int (*convert)(void*)) {
  assert(list);
  ...
  while (curr) {
     ...
     * the list.
     */
    if (convert(val) == convert(curr->val)) {
      if (prev) {
        prev->next = curr->next;
      } else {
        list->head = curr->next;
